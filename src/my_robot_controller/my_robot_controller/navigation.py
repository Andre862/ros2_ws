...
class TurtleNavigationNode(Node):
    def __init__(self):
        ...
        self.goal_poses = [  # Define goal positions and orientations
            {'x': 1.0223, 'y': -1.0740, 'yaw': -30},
            {'x': 1.6782, 'y': -5.3764, 'yaw': 60},
            {'x': -7.3860, 'y': 1.2437, 'yaw': 0},
            {'x': -7.8569, 'y': -5.1681, 'yaw': 90}
        ]
    ...
    def odom_callback(self, msg: Odometry):
        ...
        self.get_logger().info(f"Distance to goal: {distance_to_goal:.3f}")
        if distance_to_goal < 1.6:  # Threshold to consider the goal reached
